      ...
      <ul>
        <li v-for="playlist in playlists" :key="playlist.id">
          ...
          <div>
            ...
            <button @click="get_playlist_item">See playlist</button>
          </div>
        </li>
      </ul>
    </section>
    <section v-if="playlist_item && playlist_item.length" id="playlist_item">
      <h2>Your Playlist</h2>
      <ul>
        <li v-for="playlist in playlist_item" :key="playlist.id">
          <img v-if="playlist.images && playlist.images[0]" :src="playlist.images[0].url" alt="Playlist Cover" width="50" height="50" />
          <div>
            <strong>{{ playlist.track.name }}</strong> by {{ playlist.track.artists.map(artist => artist.name).join(', ') }}
            <br />
          </div>
        </li>
      </ul>
...
const playlist_item = ref<any[] | null>(null);
const uri2 = ref<string | null>(null);
const editplaylist = ref<boolean | null>(null);
...
const get_playlist_item = async () => {
  ...
  const data = await result.json();
  playlist_item.value = data.items || []; // Stocke les playlists récupérées
  console.log(playlists.value); // Affiche les playlists dans la console
};
...
#profile, #search-results, #playlists {
  width: 100%;
  max-width: 600px;
}
</style>
